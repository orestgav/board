...
// Найтовстіший він посередині й тримається таким до останнього: поки картку
// видно бодай позначкою, кільце тут єдине, що її показує. Стоншується воно
// аж на самому кінці, коли картка вже менша за десяток пікселів: вісім
// пікселів обводу роблять із такої цятки пляму, і міста за тими плямами на
// карті світу не видно. Пороги задані часткою екрана, а картки на дошці
// вищі, ніж ширші, тож поріг 0.012 — це приблизно десять пікселів заввишки.
export function locationBorderScreenWidth(width, height, scale, viewportWidth, viewportHeight) {
  ...
  const thinnestCoverage = 0.003;
  const fullWidthCoverage = 0.012;
